import { z } from "zod";
import { clientServices, companies, contacts, contracts, services } from "@/db/schema";

/** Pure domain rules for Client 360 — see docs/features/client-360.md. */

export const CLIENT_STATUSES = companies.status.enumValues;
...
export function buildClientAlerts(input: {
  ...
  const base = `/companies/${input.companyId}`;
