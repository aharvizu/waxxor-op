...
export async function getClientSummary(orgId: number, companyId: number) {
  const now = new Date();
  const [workAgg, timeAgg, convAgg, lastTouch, activeServices, activeContracts, notesCount] =
    await Promise.all([
      db
        ...
        .where(and(eq(workItems.organizationId, orgId), eq(workItems.companyId, companyId))),
      db
        .select({
      ...
      db
        .select({ n: sql<number>`count(*)::int` })
        .from(clientNotes)
        .where(and(eq(clientNotes.organizationId, orgId), eq(clientNotes.companyId, companyId))),
    ]);
  return {
    ...workAgg[0],
    ...timeAgg[0],
...
export async function getClientContacts(orgId: number, companyId: number) {
  return db
    .select()
    .from(contacts)
    .where(and(eq(contacts.organizationId, orgId), eq(contacts.companyId, companyId)))
...
export async function getClientContracts(orgId: number, companyId: number) {
  return db
    .select()
    .from(contracts)
    .where(and(eq(contracts.organizationId, orgId), eq(contracts.companyId, companyId)))
...
export async function getClientConversations(orgId: number, companyId: number, limit = 20) {
  ...
  return db
    ...
    .leftJoin(users, eq(workItems.assigneeId, users.id))
    .where(and(eq(conversations.organizationId, orgId), eq(conversations.companyId, companyId)))
...
export async function getClientNotes(orgId: number, companyId: number) {
  return db
...
export async function getClientProjects(orgId: number, companyId: number) {
  return db
    .select({
      id: projects.id,
    ...
    .where(and(eq(projects.organizationId, orgId), eq(projects.companyId, companyId)))
 ...
 */
export async function getClientAuditTrail(orgId: number, companyId: number, limit = 100) {
  const [contactIds, serviceIds, contractIds, noteIds] = await Promise.all([
    db.select({ id: contacts.id }).from(contacts).where(eq(contacts.companyId, companyId)),
    db.select({ id: clientServices.id }).from(clientServices).where(eq(clientServices.companyId, companyId)),
    db.select({ id: contracts.id }).from(contracts).where(eq(contracts.companyId, companyId)),
    db.select({ id: clientNotes.id }).from(clientNotes).where(eq(clientNotes.companyId, companyId)),
  ]);
  ...
  return db
    ...
    .where(
      and(
        ...
        or(
          and(eq(auditLogs.entityType, "client"), eq(auditLogs.entityId, companyId)),
          scoped(
            "contact",
...
export async function getClientReports(orgId: number, companyId: number) {
  return db
    .select()
    .from(reports)
    .where(and(eq(reports.organizationId, orgId), eq(reports.companyId, companyId)))
