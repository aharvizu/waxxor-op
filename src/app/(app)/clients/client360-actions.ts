  ...
  try {
    await db.transaction(async (tx) => {
      ...
      const [work] = await tx
        ...
        .from(sql`work_items`)
        .where(sql`client_id = ${client.id}`);
      if (work.n > 0) {
        throw new RuleError(
          "Este cliente tiene trabajo registrado — archívalo en lugar de eliminarlo.",
        );
      }
      await tx.delete(clients).where(eq(clients.id, client.id)); // contacts/services/contracts/notes cascade
...
const CONTACT_AUDITED = [
  "firstName", "lastName", "jobTitle", "email", "phone", "mobile",
...
async function makePrimary(tx: DbExecutor, user: SessionUser, clientId: number, contactId: number) {
  await tx
    .update(contacts)
  ...
  await recordAudit(tx, {
    ...
    oldValue: "false",
    newValue: "true",
    metadata: { event: "primary_contact_changed", clientId },
  });
}
...
export async function createContact(
  ...
  try {
    await db.transaction(async (tx) => {
      await loadClient(tx, user, data.clientId);
      ...
      await recordAudit(tx, {
        ...
        entityId: created.id,
        action: "create",
        metadata: { clientId: data.clientId, values: { ...data, clientId: undefined } },
      });
      if (data.isPrimary) await makePrimary(tx, user, data.clientId, created.id);
    });
  } catch (err) {
    return fail(err);
  }
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      if (data.isPrimary && !before.isPrimary) {
        await makePrimary(tx, user, before.clientId, before.id);
      }
    });
  } catch (err) {
    return fail(err);
...
export async function setPrimaryContact(
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      await makePrimary(tx, user, contact.clientId, contact.id);
    });
  } catch (err) {
    return fail(err);
  }
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      await recordAudit(tx, {
        ...
        entityId: contact.id,
        action: "delete",
        metadata: { values: { name: `${contact.firstName} ${contact.lastName}`, clientId: contact.clientId } },
      });
    });
  } catch (err) {
    return fail(err);
...
export async function updateClientService(
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      if (!before) throw new NotFoundError();
      const patch = { ...data, id: undefined, clientId: undefined };
      const changes = diffFields(
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      await recordAudit(tx, {
        ...
        action: "delete",
        metadata: { values: { name: contract.name, clientId: contract.clientId } },
      });
    });
  } catch (err) {
    return fail(err);
    ...
    await db.transaction(async (tx) => {
      if (data.source === "client_service") {
        ...
        await recordAudit(tx, {
          ...
          metadata: { event: "renewal_updated", clientId: before.clientId },
        });
      } else {
        const [before] = await tx
          .select()
