...
export const metadata: Metadata = { title: "Configuración · Clientes" };
...
export default async function ClientsSettingsPage() {
  ...
  const [defaults, internalUsers, defaultSlas, categories, tags] = await Promise.all([
    getSetting(user.organizationId, "clients.defaults"),
    db
    ...
    getCatalog(user.organizationId, "client_category", { includeInactive: true }),
    getCatalog(user.organizationId, "client_tag", { includeInactive: true }),
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <Card className="p-5">
          <CardHeader
            ...
            description="Se preseleccionan al crear un cliente nuevo; siempre editables por cliente."
          />
          <SettingSectionForm settingKey="clients.defaults">
            <div>
        ...
        <Card className="p-5">
          <CardHeader
            ...
            description="El SLA por defecto es por prioridad de ticket y se administra en /sla (solo SuperAdmin, regla R7)."
          />
          ...
          <p className="mt-3 text-xs text-muted">
            ...
            <Link href="/sla" className="text-primary hover:underline">Administrar SLA →</Link>
          </p>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <Card className="p-5">
          <CardHeader
            title={CATALOG_KINDS.client_category.label}
            description={CATALOG_KINDS.client_category.note}
          />
          <CatalogManager
            kind="client_category"
            items={categories}
            hasChildren={false}
            childLabel={null}
        ...
        <Card className="p-5">
          <CardHeader
            title={CATALOG_KINDS.client_tag.label}
            description={CATALOG_KINDS.client_tag.note}
          />
          <CatalogManager
            kind="client_tag"
            items={tags}
