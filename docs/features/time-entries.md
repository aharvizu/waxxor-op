...
| `time_type` | `text`, catalog-driven since 2026-07-28 (`catalog_items` kind `time_entry_type`, Settings → Actividades → Tipos de trabajo) — not a Postgres enum, admins can add/rename/recolor/deactivate values. Validated per-org in `createTimeEntry`/`updateTimeEntry`. Every org is seeded with the original 12 values (technical_work, remote_support, onsite_support, travel, waiting_customer, waiting_provider, research, documentation, meeting, training, administration, commercial); default `technical_work`, which is system-protected (can't be renamed or deleted — it's the column's hard default). |
