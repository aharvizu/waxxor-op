...
| `time_type` | `text`, catalog-driven since 2026-07-28 (`catalog_items` kind `time_entry_type`, Settings → Actividades → Tipos de trabajo) — not a Postgres enum, admins can add/rename/recolor/deactivate values. Validated per-org in `createTimeEntry`/`updateTimeEntry`. Since 2026-07-30 this is the **same catalog** `activities.activity_type` uses (`docs/features/activities.md`) — the former separate `activity_type` kind was merged in, deduplicating names both lists already shared (meeting, research, documentation, training, commercial). Default `technical_work`, which — along with `general`, `meeting` and `reminder` (carried over from the old activity-type catalog) — is system-protected. |
