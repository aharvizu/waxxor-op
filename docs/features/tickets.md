...
| status: **new · assigned · in_progress · waiting_customer · waiting_third_party · scheduled · resolved · pending_confirmation · closed · reopened · cancelled** | `category` — strict catalog dropdown (Settings → Tickets → Categorías), required and validated server-side everywhere it's set (create, resolve, edit details); `subcategory` stays free text |
...
### Dynamic Status/Priority/Billing Status catalogs (2026-07-22)

Estado, Prioridad y Estatus de cobro son ahora **catálogos dinámicos por organización** (`ticket_statuses`, `ticket_priorities`, `ticket_billing_statuses` — Configuración → Tickets), no enums fijos. Administrador y SuperAdmin pueden crear/editar/renombrar/reordenar/activar-desactivar/definir predeterminado/duplicar/eliminar valores, con reasignación obligatoria de tickets antes de eliminar uno en uso. Los 11+4+8 valores originales se conservan como filas de sistema (`isSystem`, no eliminables, con `semanticKey` inmutable); los valores personalizados solo requieren una **categoría semántica** (`open/in_progress/waiting/resolved/closed/cancelled` para Estado; equivalente de 6 categorías para Estatus de cobro). `canTransition`/`isSlaPauseStatus`/la resolución de SLA operan sobre `semanticKey`/categoría/`priorityId`, nunca sobre el texto visible — así un Estado personalizado en categoría "en espera" pausa el SLA exactamente igual que Waiting customer/Waiting third party. Los enums Postgres originales (`work_items.status`, `work_items.priority`, `tickets.billing_status`) se conservan como espejo interno sincronizado (nunca se muestran ni editan directamente) porque son un tipo cerrado de Postgres y `work_items.status/priority` es compartido con Activities (sin cambios). Ver `src/lib/ticket-catalogs.ts`.
